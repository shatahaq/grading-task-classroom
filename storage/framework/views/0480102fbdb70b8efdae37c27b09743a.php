<?php $__env->startSection('content'); ?>
    ...
    <?php if(count($courses)): ?>
        <section class="grid gap-6 sm:grid-cols-3 xl:grid-cols-4 mb-6">
            ...
            <div class="paper-sheet p-6 sm:p-8 rounded-[1rem] relative flex flex-col justify-between">
                ...
                <p class="mt-2 text-5xl font-extrabold tracking-tight text-paper-ink" data-count-up="<?php echo e(count($courses)); ?>"><?php echo e(count($courses)); ?></p>
            </div>
        </section>
        ...
        <section aria-label="Daftar kelas Google Classroom" class="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            <?php $__currentLoopData = $courses; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $course): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                ...
                <a href="<?php echo e(route('courses.show', ['courseId' => $course['id']])); ?>" class="group block h-full outline-none">
                    <article class="paper-sheet flex h-full flex-col rounded-[1rem] p-6 transition-all duration-300 group-hover:-translate-y-1 group-hover:shadow-[0_24px_54px_rgba(30,58,138,0.16),_0_3px_10px_rgba(30,58,138,0.08)] group-focus:ring-4 group-focus:ring-paper-marker/30">
                        <header class="flex items-start justify-between gap-4">
                            <div class="min-w-0">
                                ...
                                <h2 class="truncate font-serif text-xl font-bold text-paper-ink group-hover:text-paper-marker-dark transition-colors"><?php echo e($course['name']); ?></h2>
                                <p class="mt-1 truncate text-xs font-medium text-paper-ink/60"><?php echo e($course['section'] ?? 'Kelas Google'); ?> · 2026</p>
                            </div>
                        ...
                        <div class="mt-6 mb-4 flex flex-wrap gap-4 text-sm font-bold text-paper-ink/80">
                            <span class="flex items-center gap-2">
                                ...
                                <span><?php echo e($students ?? '-'); ?></span> mahasiswa
                            </span>
                            <span class="flex items-center gap-2">
                                <i data-lucide="clipboard-list" class="h-4 w-4 opacity-50"></i>
                                <span><?php echo e($assignmentCount ?? '-'); ?></span> tugas
                            </span>
                        </div>
                        ...
                        <footer class="mt-auto flex items-center justify-between gap-2 border-t border-paper-line/50 pt-4">
                            <p class="flex items-center gap-2 text-xs font-medium text-paper-ink/60">
                                ...
                                <span class="truncate"><?php echo e($lastGraded ? 'AutoGrade '.$lastGraded : 'Belum ada grading'); ?></span>
                            </p>
    ...
    <?php else: ?>
        <section class="flex min-h-[16rem] flex-col items-center justify-center p-8 text-center text-paper-ink">
            ...
            <a href="<?php echo e(route('auth.google.redirect')); ?>" class="group relative mt-6 inline-flex items-center justify-center bg-transparent px-6 py-3 text-sm font-bold text-paper-ink">
...
], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\PW-2\Projek_AI\resources\views/courses/index.blade.php ENDPATH**/ ?>