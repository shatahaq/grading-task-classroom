...
<head>
    ...
    <title>{{ $title ?? 'AutoGrade AI' }}</title>
        ...
        <aside data-sidebar
            ...
            <nav class="mt-10 grid gap-2" aria-label="Navigasi utama">
                @foreach ($navItems as $item)
                    <a href="{{ route($item['route']) }}"
                        class="group flex items-center gap-3 rounded-xl px-4 py-3 text-sm font-extrabold transition-all duration-300 focus:outline-none focus:ring-4 focus:ring-paper-blue/60 {{ $item['active'] ? $activeNav : $idleNav }}"
            ...
            <section
                ...
                <div class="flex items-center gap-3">
                    <span
                        class="sketch-avatar flex h-14 w-14 shrink-0 items-center justify-center rounded-full font-serif text-sm font-bold text-paper-ink">{{ $initials }}</span>
                    <span class="min-w-0">
                        <span class="block truncate text-sm font-extrabold text-paper-ink">{{ $displayName }}</span>
                        @if ($displayEmail)
                            <span class="block truncate text-xs font-medium text-paper-muted">{{ $displayEmail }}</span>
                        @endif
                    </span>
                </div>
            ...
            <header class="sticky top-0 z-30 bg-paper-bg/75 px-5 py-5 backdrop-blur-md sm:px-8">
                <div class="flex items-center justify-between gap-4">
                    ...
                    <span
                        ...
                        <i data-lucide="{{ $connected ? 'plug-zap' : 'circle-alert' }}" class="h-4 w-4"></i>
                        Google {{ $connected ? 'Tersambung' : 'Belum' }}
                    </span>
                </div>
            </header>
