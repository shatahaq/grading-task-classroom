    ...
    <section class="grid gap-6">
        <article class="paper-sheet p-8 sm:p-10 mb-2">
            <div class="relative z-10">
                ...
                <h1 class="mt-6 max-w-3xl font-serif text-3xl font-bold tracking-tight text-paper-ink sm:text-4xl">
                    {{ $greeting }}, {{ auth()->user()?->name ?? '061_Shata Diyaul Haq' }}.
                </h1>
                ...
                <div class="mt-8 flex flex-wrap gap-4">
                    <a href="{{ route('assignments.create') }}" class="group relative inline-flex items-center justify-center bg-transparent px-6 py-2.5 text-sm font-bold text-paper-ink">
                    ...
                    <a href="{{ route('courses.index') }}" class="group relative inline-flex items-center justify-center bg-transparent px-6 py-2.5 text-sm font-bold text-paper-ink">
                        <span class="absolute inset-0 rounded-sm bg-white shadow-[0_2px_4px_rgba(30,58,138,0.1),_0_0_0_1px_rgba(30,58,138,0.05)] transition-all group-hover:-rotate-1 group-hover:scale-105"></span>
        ...
        <aside class="grid gap-4 sm:gap-6 md:grid-cols-3">
            <article class="paper-sheet p-6 relative">
                ...
                <p class="mt-2 text-5xl font-extrabold tracking-tight text-paper-ink">{{ $stats['review'] ?? 0 }}</p>
            ...
            <article class="paper-sheet p-6 relative">
                ...
                <p class="mt-2 text-5xl font-extrabold tracking-tight text-paper-ink">{{ $stats['courses'] ?? 0 }}</p>
            </article>
            <article class="paper-sheet p-6 relative">
                <div class="paper-clip"></div>
                <p class="text-[11px] font-bold uppercase tracking-widest text-paper-ink/70">Disetujui</p>
                <p class="mt-2 text-5xl font-extrabold tracking-tight text-paper-ink">{{ $stats['approved'] ?? 0 }}</p>
            </article>
        </aside>
    </section>
    ...
    <section class="mt-8 mb-10">
        <section class="paper-sheet rounded-[1rem]">
            <header class="flex flex-col gap-4 border-b border-paper-line/50 p-6 md:flex-row md:items-center md:justify-between">
                ...
                <a href="{{ route('assignments.create') }}" class="group relative inline-flex items-center justify-center bg-transparent px-5 py-2 text-sm font-bold text-paper-ink">
                    <span class="absolute inset-0 rounded-sm bg-white shadow-[0_2px_4px_rgba(30,58,138,0.1),_0_0_0_1px_rgba(30,58,138,0.05)] transition-all group-hover:-rotate-1 group-hover:scale-105"></span>
                            ...
                            @foreach ($recentAssignments as $assignment)
                                <tr class="hover:bg-paper-blue/10 transition-colors">
                                    <td class="py-4 px-6 font-medium text-paper-ink">
                                        <a href="{{ route('assignments.grading', $assignment) }}" class="hover:underline">{{ $assignment->title }}</a>
                                    </td>
                                    <td class="py-4 px-6 font-medium text-paper-ink/80">{{ $assignment->created_at?->format('d M Y') ?? 'N/A' }}</td>
                                    <td class="py-4 px-6 font-bold text-paper-ink">
                                        <span class="marker-highlight px-2 py-0.5 inline-block">{{ $statusLabel($assignment->status) }}</span>
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
            ...
            @else
                <section class="flex min-h-[16rem] flex-col items-center justify-center p-8 text-center text-paper-ink">
                    ...
                    <a href="{{ route('assignments.create') }}" class="group relative mt-6 inline-flex items-center justify-center bg-transparent px-6 py-3 text-sm font-bold text-paper-ink">
