...
@section('content')
    ...
    <section class="paper-sheet p-6 sm:p-8 rounded-[1rem] mb-6 flex flex-col md:flex-row md:items-start md:justify-between gap-6">
        <div>
            <a href="{{ route('courses.show', ['courseId' => $assignment->course_id]) }}" class="inline-flex items-center gap-2 text-sm font-bold text-paper-ink/60 transition-colors hover:text-paper-ink mb-4">
                <i data-lucide="arrow-left" class="h-4 w-4"></i>
                Kembali
            </a>
            <div class="flex flex-wrap items-center gap-4">
                <h1 class="font-serif text-3xl font-bold tracking-tight text-paper-ink">{{ $assignment->title }}</h1>
                <span id="assignment-status-badge" class="marker-highlight inline-flex items-center gap-2 px-3 py-1 text-[11px] font-bold uppercase tracking-wider text-paper-ink">
                    <i data-lucide="{{ $statusMeta['icon'] }}" class="h-4 w-4"></i>
                    <span id="assignment-status">{{ $statusMeta['label'] }}</span>
                </span>
            </div>
            <div class="mt-4 flex flex-wrap items-center gap-x-6 gap-y-2 text-sm font-medium text-paper-ink/80">
                <span class="flex items-center gap-2"><i data-lucide="graduation-cap" class="h-4 w-4 opacity-70"></i>{{ $assignment->course_id }}</span>
                <span class="flex items-center gap-2"><i data-lucide="award" class="h-4 w-4 opacity-70"></i>Maks {{ number_format((float) $assignment->max_score, 0) }}</span>
                <span class="flex items-center gap-2"><i data-lucide="users" class="h-4 w-4 opacity-70"></i>{{ $results->count() }} mahasiswa</span>
                <span class="flex items-center gap-2"><i data-lucide="calendar-clock" class="h-4 w-4 opacity-70"></i>{{ $assignment->due_date?->translatedFormat('d M Y') ?? 'Tanpa tenggat' }}</span>
            </div>
        </div>
        ...
        @foreach ([
            ...
            <article class="paper-sheet p-6 relative flex flex-col justify-between">
                ...
                <div class="flex items-center justify-between mb-4">
                    <p class="text-[11px] font-bold uppercase tracking-widest text-paper-ink/70">{{ $stat['label'] }}</p>
                    <i data-lucide="{{ $stat['icon'] }}" class="h-5 w-5 text-paper-ink/40"></i>
                </div>
                <p class="text-5xl font-extrabold tracking-tight text-paper-ink" data-count-up="{{ $stat['value'] }}" data-decimals="{{ $stat['decimals'] }}">{{ $stat['value'] }}</p>
            </article>
        @endforeach
    </section>
...
@push('scripts')
    <script>
        const maxScore = Number(@json((float) ($assignment->max_score ?: 100)));
        let rows = @json($gradingRows);
        ...
        button.addEventListener('click', async () => {
            ...
            try {
                const response = await fetch(@json(route('assignments.grading.trigger', $assignment)), {
                    method: 'POST',
